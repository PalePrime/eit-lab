...
static uint pwmDmaBufCh; // This DMA channel will feed buffer addresses to the
                         // first DMA channel
static volatile uint32_t
    *pwmDataPtr;                     // Memory address controlling the pwm level
static volatile uint32_t *pwmDivPtr; // Memory address controlling the pwm clock
                                     // rate by setting a divider
...
static void pwmDmaHandler() {
  ...
  if (spkChannel.state.state != AUDIO_IDLE) {
    ...
    if (controlMsgFromISR(&spkChannel, CH_DATA_SENT,
                          spkChannel.state.ioChunk) != pdPASS) {
...
static void spkCodecTask(void *pvParameters) {
  ...
  int16_t bufLeft[MAX_IO_CHUNK];
  while (true) {
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
    loop = true;
    while (loop) {
      uint32_t availableSamples = tud_audio_available() >> 2;
      // Buffer reports # of bytes, there are four bytes per sample
      if (availableSamples > MAX_IO_CHUNK) {
        availableSamples = MAX_IO_CHUNK;
      } else {
        loop = false;
      }
      tud_audio_read(buf, availableSamples << 2); // Read four bytes per sample
      for (uint32_t i = 0; i < availableSamples; i++) {
        // For now we just pick samples from first (left) channel
        int16_t sample = buf[i << 1];
        sample = sample >> 8;
        // -128 -- +127 in low byte, sign in high byte
        sample = (sample + 128) & 0xff;
        //    0 --  255 in low byte, zero in high byte
        bufLeft[i] = sample;
      }
      // Push decoded samples to output FIFO
      if (tu_fifo_write_n(&dataOut, bufLeft, availableSamples) !=
          availableSamples) {
...
static void setSpkDiv(uint32_t clockDiv) { *pwmDivPtr = (clockDiv >> 4); }
...
static usb_channel_settings_t spkChSettings = {.idStr = "Spk",
                                               .toUsb = false,
                                               .setDiv = setSpkDiv,
                                               .open = openSpkCh,
                                               .close = closeSpkCh,
...
void createSpkChannel() {
  ...
  pwmSlice = pwm_gpio_to_slice_num(
      PWM_PIN);                // Find out which pwm slice controls the pin
  ...
  *pwmDivPtr = pwmDiv >> 4;                 // Set the initial sane value
  ...
  pwmDmaCh = dma_claim_unused_channel(
      true); // One to actually push data from buffer to pwm
  pwmDmaBufCh = dma_claim_unused_channel(
      true); // and one to switch the buffer address in the first channel

  // The whole point of this setup is to allow some delay in executing the
  // interrupt handler for DMA there is no hardware fifo in the pwm slice so new
  // data has to be written before the sample interval ends, for 48 kHz this
  // means that once a buffer is tranferred, the next has to start within 20 us.
  // With only the data DMA channel running and buffer changes performed by an
  // interrupt handler this is difficult i view of other interrupt handlers also
  // running, servicing e.g. the USB hardware. Having a second DMA channel
  // update the buffer address of the data channel losens the real time demands
  // a lot. Basically the second DMA swaps the buffer and then interrupts the
  // CPU. This means that the only requirement on the interrupt is to update the
  // buffer and re-arm the second DMA channel before it completes sending all
  // data in the buffer.
  ...
  channel_config_set_transfer_data_size(
      &c1, DMA_SIZE_16); // Feed 16-bit data to the pwm slice
  channel_config_set_write_increment(
      &c1, false); // to the fixed pwm slice data location
  channel_config_set_read_increment(
      &c1, true); // while incrementing the buffer address
  channel_config_set_dreq(
      &c1,
      pwm_get_dreq(pwmSlice)); // repeating each time one pwm cycle completes
  channel_config_set_chain_to(
      &c1, pwmDmaBufCh); // and finally trigger the second DMA channel
  ...
  dma_channel_configure(
      ...
      24,   // Number of transfers (just a sane initial value, set when sample
            // rate is adjusted)
  ...
  channel_config_set_transfer_data_size(
      &c2, DMA_SIZE_32); // Feed 32-bit memory adresses to the first DMA channel
  channel_config_set_write_increment(
      &c2, false); // to the fixed DMA controller address
  channel_config_set_read_increment(&c2, false); // from a fixed location

  dma_channel_configure(
      pwmDmaBufCh, &c2,
      &dma_channel_hw_addr(pwmDmaCh)
           ->al3_read_addr_trig, // Write next buffer pointer into read address
                                 // reg of the first DMA channel
  ...
  dma_channel_set_irq1_enabled(
      pwmDmaBufCh,
      true); // Use interrupt vector DMA_IRQ_1, the mic channel uses DMA_IRQ_0
  irq_set_exclusive_handler(DMA_IRQ_1,
                            pwmDmaHandler); // Connect the vector to our ISR
  irq_set_enabled(DMA_IRQ_1, true); // and enable interrupts on that vector

  spk_codec_handle =
      xTaskCreateStatic(spkCodecTask, "Spk codec", SPK_CODEC_STACK_SIZE, NULL,
                        SPK_CODEC_TASK_PRIO, spk_codec_stack, &spk_codec_def);
...
bool tud_audio_rx_done_pre_read_cb(uint8_t rhport, uint16_t n_bytes_received,
  ...
  //  pinState = !pinState;
  //  gpio_put(DBG_PIN, pinState);
  if (controlMsg(&spkChannel, CH_DATA_RECEIVED, n_bytes_received >> 2) !=
      pdPASS) {
